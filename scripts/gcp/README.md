...
| `setup-item-encryption-key-secret.sh` | アイテム本文用 `ITEM_ENCRYPTION_KEY`（32 バイト base64）を `vault-share-item-encryption-key` に保存し IAM を付与   |
...
GitHub Actions の `deploy.yml` は **`vault-share-item-encryption-key` を `ITEM_ENCRYPTION_KEY` として参照**する。未作成だとデプロイが失敗する。初回は `setup-item-encryption-key-secret.sh` を実行すること（既存のローカルキーを `ITEM_ENCRYPTION_KEY` で渡してもよい）。
